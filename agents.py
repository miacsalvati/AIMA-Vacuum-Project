...
class Environment:
    ...
    def list_things_at(self, location, tclass=Thing):
        """Return all things exactly at a given location.
           Walls while things can't be at a location """
...
class GraphicEnvironment(XYEnvironment):
    ...
    def draw_world(self):
        ...
        for x in range(0, len(world)): 
            self.grid[0, x]=self.colors['Wall']
            self.grid[len(world[x])-1, x]=self.colors['Wall']
        for y in range(0, len(world[0])):
            self.grid[y, 0]=self.colors['Wall']
            self.grid[y, len(world)-1]=self.colors['Wall']
